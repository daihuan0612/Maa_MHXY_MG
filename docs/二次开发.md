...
6. 发版您的版本
    ...
    git tag  -a v1.2.9 -m ""1.修复日常任务，2.修复抓鬼任务。[已有 Mirror酱 CDK？点击前往高速下载](https://mirrorchyan.com/zh/projects?rid=Maa_MHXY_MG)"
    
    # 临时禁用 SSL 验证 并push代码
    git -c http.sslVerify=false push origin HEAD -u
    # 临时禁用 SSL 验证 并push tag
    git -c http.sslVerify=false push origin v1.2.9
